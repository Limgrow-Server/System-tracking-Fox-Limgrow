...
function topicSlug(appId: string) {
  return appId
    .toLowerCase()
    .replace(/[^a-z0-9-_.~%]+/g, "-")
    .replace(/^-+|-+$/g, "")
...
function suggestedTopic(appId: string, language: string) {
  return `${topicSlug(appId)}-${language === "default" ? "all" : language}`;
}
...
export function NotificationConfigPage({ apps }: { apps: AppConfigOption[] }) {
  ...
  async function saveConfig(event: FormEvent) {
    ...
    const topics = topicRows.map((row) => row.topic.trim().replace(/^\/topics\//i, ""));
    if (new Set(languages).size !== languages.length) {
      await showToast("warning", "Mỗi ngôn ngữ chỉ được cấu hình một lần.");
      return;
    }
    if (new Set(topics).size !== topics.length) {
      await showToast("warning", "Mỗi ngôn ngữ cần một Firebase topic riêng.");
      return;
    }
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <Input
                            ...
                            placeholder={`${topicSlug(selectedApp.appId)}-${row.language}`}
                            required={enabled}
                          />
                        </div>
                      </div>
